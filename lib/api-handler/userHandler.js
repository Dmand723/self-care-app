...
export const userContex = createContext({
  ...
  removeStars: async () => {},
  addTodo: async (title) => {},
  setTodoCompleted: async (title, value) => {},
  removeTodo: async (title) => {},
});
export default function UserProviderHandler({ children }) {
  ...
  const removeTodo = async (title) => {
    ...
    const docRef = doc(db, "users", usersData.id);
    const existingTodo = usersData.todo || [];
    const updatedTodo = existingTodo.filter((todo) => {
      return todo.title !== title;
    });
    await updateDoc(docRef, { todo: updatedTodo });
  };

  const setTodoCompleted = async (title, value) => {
    const usersData = uData;
    const docRef = doc(db, "users", usersData.id);

    // Update the specific todo item in the array
    const updatedTodos = usersData.todo.map((todo) =>
      todo.title === title ? { ...todo, completed: value } : todo
    );

    // Update Firestore with the modified array
    await updateDoc(docRef, { todo: updatedTodos });
  };
